...
- 🎯 **NEW! Kindle совместимость**: EPUB файлы оптимизированы для Amazon Kindle
- 📝 **NEW! Умные названия файлов**: Автоматическое добавление суффикса "_Конвертовано"
...
### 🔧 Исправление ошибки E999 на Kindle (v2.1.0)

Бот теперь создает EPUB файлы, полностью совместимые с Amazon Kindle:

- ✅ **EPUB 2.0 формат** для максимальной совместимости
- ✅ **Встроенное оглавление** и оптимизированная структура
- ✅ **Корректные метаданные** по умолчанию
- ✅ **Плоская структура файлов** для лучшей поддержки

### 📝 Улучшенные названия файлов

Новая схема именования для конвертированных файлов:
- **Формат**: `ОригинальноеИмя_Конвертовано.формат`
- **Очистка символов**: Автоматическое удаление проблемных символов
- **Примеры**: 
  - `Война и мир.pdf` → `Война_и_мир_Конвертовано.epub`
  - `Document<test>.txt` → `Documenttest_Конвертовано.mobi`

Подробнее в [KINDLE_FIX.md](KINDLE_FIX.md)
...
├── KINDLE_FIX.md        # Документация исправлений
└── update_fixes.sh      # Быстрое обновление на сервере
```
...
### Тестирование Kindle совместимости
1. Отправьте HTML файл боту через `test_kindle_book.html`
2. Конвертируйте в EPUB
3. Загрузите файл на Kindle устройство
4. Убедитесь, что файл открывается без ошибки E999
